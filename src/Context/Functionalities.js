...
const IfMovieLiked = (movie) => {
  const { linkedMovies } = useSelector((state) => state.userGetFavorite);
  return linkedMovies?.find((linkedMovie) => linkedMovie?._id === movie?._id);
};
...
//download video url function
const DowloadVideo = async (videoUrl, setProgress) => {
...
export { IfMovieLiked, likeMovie ,DowloadVideo};
